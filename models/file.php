<?php 
	...
	class File {
		...
		public function upload_file($file, $title, $description) {
			$path = $this->upload_dir.$name;
			$name = $file['name'];
			...
			// if (!move_uploaded_file($file['tmp_name'], $this->upload_dir.$uniqueName.$type)) {
			// 	throw new Exception('Error uploading file. Please try again later');
			// } 
			
			// Add file to the database 
			$sql = 'CALL add_image(:title, :path, :description)';
			
			try {
				$stmt = $this->gateway->dbh->prepare($sql);
				$stmt->bindParam(':title', $title);
				$stmt->bindParam(':path', $uniqueName);
